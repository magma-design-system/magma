...
const logDirectoryCopied = (directory: PathLike, destination: PathLike) => {
  console.info(
    `Directory ${chalk.yellow(path.basename(directory.toString()))} successfully ${chalk.blue('copied')} in ${chalk.yellow(path.basename(destination.toString()))}`,
  );
};

const logDirectoryCreated = (directory: PathLike, destination?: PathLike) => {
  console.info(
    `Directory ${chalk.yellow(path.basename(directory.toString()))} successfully ${chalk.green('created')} ${destination ? 'in ' + chalk.yellow(path.basename(destination.toString())) : ''}`,
  );
};

const logDirectoryDeleted = (directory: PathLike) => {
  console.info(
    `Directory ${chalk.yellow(path.basename(directory.toString()))} successfully ${chalk.red('deleted')} ${chalk.gray('(or skipped if missing)')}`,
  );
};
...
const logFileSavedTo = async (file: PathLike, destination?: PathLike) => {
  ...
  if (destination) {
    ...
    console.info(
      `File ${chalk.yellow(cleanFile)} successfully ${chalk.green('saved')} in ${chalk.yellow(cleanDestination)}`,
    );
  } else {
...
const logFileActionDone = (args: FileActionDoneOptions) => {
  console.info(
    `${capitalize(args.entity.trim())}${args.source ? ' ' + chalk.yellow(path.basename(args.source.toString().trim())) : ''} successfully ${chalk.blue(args.actionDone.trim())} ${args.destination ? `as ${chalk.yellow(path.basename(args.destination.toString().trim()))}` : ''}`,
  );
};
...
const logStatus = (args: StatusOptions) => {
  console.info(
    `${capitalize(args.actionDoing)} in ${args.subject ? chalk.yellow(args.subject) : ''} ${args.status ? args.status : ''} ${args.match ? chalk.yellow(args.match) : ''}`,
  );
};
