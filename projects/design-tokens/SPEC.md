...
family and mode, `theme.text` declares an APCA Lc target per role against that
family's `surface-default`; the engine (`src/lib/text-role.mts`) picks the
LEAST-contrast tone step that still meets the target - the most de-emphasized
shade that stays legible. An explicit `{ step }` overrides the search for exact
...
"theme": {
  "surfaces": { ... },
  "borders": { ... },
  // APCA Lc target per role vs surface-default (same magnitude both modes),
