...
## Text-role engine (by-target / A7, issue #591)

Text roles are the FOREGROUND side of the theme, so they are generated by
CONTRAST TARGET (APCA), not by lightness - consistent with the tone scale's own
philosophy (surfaces are the deliberate lightness exception). For each opted-in
family and mode, `theme.text` declares an APCA Lc target per role, checked against
the family's WORST-CASE (lowest-contrast) surface; the engine
(`src/lib/text-role.mts`) picks the LEAST-contrast tone step that still meets it
there, so the role stays legible on every surface - the most de-emphasized shade
that is legible everywhere. An explicit `{ step }` overrides the search for exact
control; an unreachable target clamps to the strongest step with a warning.

```jsonc
"theme": {
  "surfaces": { ... },
  "borders": { ... },
  // APCA Lc target per role vs the worst-case surface (same magnitude both modes),
  // or { "step": N } to pin an exact tone step
