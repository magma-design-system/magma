...
New colors are named automatically: when the color picker is released, a color still carrying a system-assigned name gets one from the "Name That Color" vocabulary (via [color-namer](https://github.com/colorjs/color-namer), ~1500 names, kebab-cased: e.g. `label.persian-green`), made unique with a numeric suffix when needed. Only names assigned by the playground itself keep following the picker: a name typed by the user, or any name coming from a loaded configuration, is never touched. Picking a value already used by another color raises a warning.
