...
con.color = (color, text = '') => {
  ...
  con.log(
    `%c${color} ${text}`,
    `background-color: ${color};padding: 5px; border-radius: 5px; color: ${l > 0.5 ? '#000' : '#fff'}`,
  );
};
...
con.ramp = (scale, length = 1) => {
  con.log(
    '%c ',
    `font-size: 1px;line-height: 16px;background: ${chromajs.getCSSGradient(scale, length)};padding: 0 0 0 200px; border-radius: 2px;`,
  );
};
...
const extendChroma = (chroma) => {
  ...
  chroma.Color.prototype.hsluv = function () {
    ...
    const rgb = this._rgb.slice(0, 3).map((c) => c / 255);
    hsluv.rgb_r = rgb[0];
    hsluv.rgb_g = rgb[1];
    hsluv.rgb_b = rgb[2];
    hsluv.rgbToHsluv();
  ...
  chroma.hsluv = (...args) =>
    new chroma.Color(...Hsluv.hsluvToRgb(args).map((c) => Math.floor(c * 255)), 'rgb');
