...
const selector = (file: PathLike): string => `mgg/${basename(file.toString()).replace('.svg', '')}`;
...
const dictionary = (icons: PathLike[]): string[] => {
  ...
  for (const icon of icons) {
    if (!/^[a-z0-9-]+\.svg$/.test(basename(icon.toString()))) {
      throw new Error(
        `Icon '${basename(icon.toString())}' contains illegal characters, you must use a-z, 0-9 and - chars.`,
      );
    }
...
const buildDictionary = async (): Promise<void> => {
  ...
  await mkdir(DIST_DIR, { recursive: true }).catch((error) => {
