...
const logFile = ({ filepath, filetype, resolution, size }: LogData) => {
...
const resource = (item: string): string => path.join(RESOURCES_PATH, `${item}.svg`);

const formatFilename = (name: string, size: number, extension: string) =>
  `${name}-${size}w.${extension}`;

const formatResolution = (width: number, height: number) => `${width}x${height}px`;
...
walk(RESOURCES_PATH, (error: string, results: string[]) => {
  ...
  cleanPaths.forEach((filePath) => {
    mkdir(path.join(DIST_DIR, filePath), { recursive: true }).catch((error) => {
      throw Error(chalk.red(error));
    });
  });
