...
const splitCSSEncoded = (
  cssBuffer: Buffer,
  fontFacePath: PathLike | fs.FileHandle,
  classesPath: PathLike | fs.FileHandle,
) => {
...
const buildCSSEncoded = (buildFontsDir: string, buildPathDir: string, fontName: string) => {
  ...
  return fs
    .mkdir(BASE64_PATH_DIR, { recursive: true })
...
const createBuildDirective = async (buildSvgDir: string): Promise<void> => {
  return fs
    .mkdir(buildSvgDir, { recursive: true })
    .then(() => {
...
const iconsToDictionary = async (buildPathDir: string, inputData: Map<string, string>) => {
  return fs
    .writeFile(path.resolve(buildPathDir, 'dictionary.json'), JSON.stringify(inputData, null, 2))
    .then(() => {
...
const iconsToTempFolder = async (
  buildSvgDir: string,
  inputData: Map<string, string>,
): Promise<void | void[]> => {
  const promises = [];
  const entries =
    Object.entries(inputData).length === 0 ? inputData.entries() : Object.entries(inputData);
  for (const [key, value] of entries) {
    const icon = iconSelectorToObject(value.toString());
    const sourcePathPromise = ICON_GROUPS[icon.group as keyof typeof ICON_GROUPS].getPath(
      icon.name,
    );
...
const buildFont = async (options: BuildFontOptions): Promise<void> => {
  ...
  return svgtofont(_options).then(() => addPrefixToAssetsUrlInScss(scssFileName, options.cssPath)); // defaultValue == cssPath
};

const addPrefixToAssetsUrlInScss = (scssFileName: PathLike, cssPath = '') => {
  return fs
    .readFile(scssFileName)
    .then((scssText) => {
      const variableName = '$font-icons-base-url';
      return `${variableName}: '${path.basename(cssPath)}/' !default;\n\n${scssText}`.replace(
        new RegExp(`url\\(('|")${path.basename(cssPath)}`, 'g'),
        `url(${variableName} + $1`,
      );
    })
...
const getSvgToFontOptions = ({
  svgPath,
  relativeOutputPath,
  cssPath,
  fontName,
  website,
}: BuildFontOptions): SvgToFontOptions => {
  return {
    src: svgPath,
    ...
    website: !website
      ...
      : {
          ...
          description:
            'Maggioli Design System Icons is an icon font package based on Google Material Design Icons and Material Design Icons community icons.',
          logo: '',
          favicon: undefined,
...
const organizeFiles = (buildFontsDir: string, buildPathDir: string) => {
  ...
  return fs
    .readdir(buildFontsDir)
    .then((files) => files.filter((filename) => !isFontFile.test(filename)))
    .then((files) =>
      Promise.all(
        files.map((file) =>
          fs.rename(path.join(buildFontsDir, file), path.join(buildPathDir, file)),
        ),
      ),
    );
};
