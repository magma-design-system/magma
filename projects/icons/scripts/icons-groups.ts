...
class MaterialIconsSvg {
  ...
  static async getPath(iconName: string): Promise<string> {
    ...
    const subdirectories = (await MaterialIconsSvg.subDirectories()).filter(
      (path) => path.match(pattern) !== null,
    );
 ...
 */
const iconGroupGetHelper = async (
  iconGroup: string,
  directories: string[],
  iconName: string,
  filename: string,
  ...
  throw new Error(
    `${chalk.red('Icon not found:')} ${iconGroup ? iconGroup + '/' : ''}${iconName}, searched as ${filename}`,
  );
};
 ...
 */
const iconGroupListHelper = async (
  directories: string[],
  fileTemplate?: RegExp,
): Promise<string[]> => {
  return Promise.all(
    directories.map(async (dir) =>
      (await listFilesInDirectory(dir, fileTemplate)).map((filename) => path.join(dir, filename)),
    ),
 ...
 */
const searchFileInDirectory = async (
  directory: string,
  filename: string,
): Promise<string | void> => {
...
const ICON_GROUPS = {
  localDirectory: {
    getPath: LocalDirectory.getPath,
    listPath: LocalDirectory.listPath,
    getIconName: LocalDirectory.getIconName,
    subDirectories: LocalDirectory._subDirectories,
  },
  maggioli: {
    getPath: Maggioli.getPath,
    listPath: Maggioli.listPath,
    getIconName: Maggioli.getIconName,
  },
  material: {
    getPath: Material.getPath,
    listPath: Material.listPath,
    getIconName: Material.getIconName,
  },
  'material-icons-svg': {
    getPath: MaterialIconsSvg.getPath,
    listPath: MaterialIconsSvg.listPath,
    getIconName: MaterialIconsSvg.getIconName,
  },
  mdi: {
    getPath: MaterialCommunity.getPath,
    listPath: MaterialCommunity.listPath,
    getIconName: MaterialCommunity.getIconName,
  },
};
