...
- **Slot-driven lifecycle**: Items placed in the default slot are animated in automatically on `slotchange`; each item is wired to its own `mdsPushNotificationItemClose` event so it animates out and is removed from the DOM when closed.
...
- **Visibility reflection**: `visible` is reflected to the host attribute and is mutable; toggling it drives the show/hide animations.
...
- **Animation timing**: Intro/outro durations and inter-item gap are read from CSS custom properties (`--mds-push-notification-items-intro-delay`, `--mds-push-notification-items-outro-delay`, `--mds-push-notification-items-gap`) at load, so motion is themeable without code changes.
