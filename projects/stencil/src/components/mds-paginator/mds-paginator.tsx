...
export class MdsPaginator {
   ...
   */
  private isKeyboardFocus = (item: HTMLElement): boolean => {
    try {
  ...
  private handleItemFocus = (ev: FocusEvent): void => {
  ...
  render () {
    return (
      <Host>
        ...
        { this.pages > 2 &&
          <div class="pages">
            { Array.from(Array(this.pages - 2).keys()).map( i => <mds-paginator-item key={i} class="item" selected={this.currentPage === i + 2} onClick={ev => this.goToPage(i + 2, ev.target as HTMLMdsPaginatorItemElement)} onFocus={this.handleItemFocus}>{ i + 2 }</mds-paginator-item>) }
          </div>
        }
