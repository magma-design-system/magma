...
The `<mds-calendar>` web component is the date-selection surface of the Magma Design System: a localized month grid that lets users pick either a single day or a date range. It is the visual engine behind `mds-input-date` and `mds-input-date-range`, and is built on Luxon for date math rather than the native `<input type="date">`.
...
- **Hover preview**: While a range start is set, hovering over cells previews the candidate range live via `preview`/`selection` attributes on `mds-calendar-cell` children (range mode only).
...
- **Localization**: Weekday names, month names, and cell titles are formatted from the host's resolved locale (`it`, `en`, `es`, `el`); `updateLang()` re-reads it at runtime.
...
- **Preselection slot**: A `preselection` named slot hosts quick-pick shortcuts; the host detects whether it contains an active preselection and reflects this to the layout.
