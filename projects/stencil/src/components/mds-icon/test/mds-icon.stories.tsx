...
const Template = args => <mds-icon {...args} />
...
const ListIconsTemplate = () => {
  const [filteredIcons, setFilteredIcons] =
    useState<string[]>(mggIconsDictionary)

  return (
    <div class="flex flex-col gap-y-400">
      <div class="flex flex-col gap-y-600">
        <mds-text>
          La seguente è una lista delle icone presenti nel pacchetto{' '}
          <strong>
            @maggioli-design-system/svg-icons {svgIconPackage.version}
          </strong>
          , facente riferimento alle icone di Maggioli
        </mds-text>
        <mds-text>
          Oltre a questo set, <strong>@maggioli-design-system/icons</strong> fa
          uso anche delle librerie di{' '}
          <a href="https://fonts.google.com/icons">Material Icons</a> e{' '}
          <a href="https://pictogrammers.com/library/mdi/">
            Material Design Icons
          </a>
          , la cui lista può essere consultata sui relativi siti.
        </mds-text>
      </div>
      <div class="flex flex-col gap-y-600">
        <mds-input
          ...
          onInput={event => {
            ...
            setFilteredIcons(
              mggIconsDictionary.filter(item => item.includes(_filter)),
            )
          }}
        />
        <div class="grid grid-cols-5 gap-400">
          {filteredIcons.map(icon => (
            <div key={icon} class="flex flex-col justify-center items-center">
