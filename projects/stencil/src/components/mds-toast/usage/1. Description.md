...
- **Close event**: After the outro animation completes (timed against the dismiss animation duration read from CSS), the component emits `mdsToastClose` — consumers should listen for this to remove the toast from the DOM or update queue state.
- **Reactive timer**: Changing `visible` or `duration` at runtime restarts the timer through watchers, so toggling visibility re-arms the countdown rather than leaving a stale timer.
- **Conditional regions**: The text region (wrapped in an `mds-text` with `caption` typography) renders only when the host has inner content, and the action region renders only when a `[slot="action"]` child is present — empty slots produce no layout.
