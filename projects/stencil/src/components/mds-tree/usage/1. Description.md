...
- **Depth seeding**: On load it finds its direct (`:scope > mds-tree-item`) children and assigns them `depth = 0`, so each branch can compute its own indentation downward from the root.
- **Stacking order**: As slotted nodes change it rewrites their `z-index` in descending order (first item highest), keeping overlapping branch decorations and hover surfaces layered correctly.
