...
export class MdsPrefThemeVariant {
   ...
   */
  @Event({ eventName: 'mdsPrefThemeVariantChange' }) themeChangeEvent: EventEmitter<MdsPrefThemeVariantEventDetail>
  ...
  componentWillRender (): void {
    ...
    this.userThemeScheme = localStorage.getItem(this.localStorageAliasThemeScheme) as PreferenceThemeSchemeType | 'all'
      ...
      <Host>
        <div class="menu">
          ...
          <mds-tab fill size={this.size}>
            <mds-tab-item selected onClick={this.toggleDropdown} id="mds-pref-theme-variant-nav" class="item item--custom-theme-variant" icon-position="right" icon={this.showDropdown ? miBaselineKeyboardArrowUp : miBaselineKeyboardArrowDown}>{this.name}</mds-tab-item>
          </mds-tab>
        </div>
        <mds-dropdown class="mds-pref-theme-variant-dropdown" target="#mds-pref-theme-variant-nav" interaction="none" visible={this.showDropdown} onMdsDropdownHide={this.hideThemeVariantDropdownSelect} autoPlacement>
          <slot></slot>
        </mds-dropdown>
        { this.name !== this.defaultTheme && <mds-text typography="caption">{ this.t.get('defaultTheme') }</mds-text> }
      </Host>
    )
  }
}
