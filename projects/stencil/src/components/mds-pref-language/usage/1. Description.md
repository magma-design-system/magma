...
- **Compound child constraint**: Must be placed as a direct slot child of `<mds-pref>`; it is not a standalone widget and shares the `mds-pref-*` family inside that parent. In turn it acts as the host for its own `<mds-pref-language-item>` children, which must be supplied via its default slot.
- **Default slot hosts the items**: The default slot accepts one or more `<mds-pref-language-item>` elements; on load the component queries them, wires a `mdsPrefLanguageItemSelect` listener on each, and marks the item whose `code` matches `set` as `selected`.
- **Selection orchestration**: Selecting an item clears `selected` on every sibling item, sets it on the chosen one, closes the dropdown, and resolves the new language — the children report up to this component, which centralizes the single-selection state.
- **Language resolution & persistence**: When `set` is `auto` the active language is resolved in priority order from `localStorage` (`mdsPrefLanguage`), then the `<html lang>` attribute, then `navigator.language`. The resolved value is written back to `localStorage` and applied to `<html lang>`, and locale-aware strings on the page are refreshed.
...
- **Size driven by the parent**: `<mds-pref>` propagates its `size` down to every `mds-pref-*` child, so this component's `size` is normally controlled by the parent rather than set directly.
