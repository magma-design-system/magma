...
export class MdsLabel {
  ...
  render() {
    return (
      <Host>
        <mds-text typography={this.typography} class={clsx('flex-grow min-w-0', this.truncate && 'truncate')}>
          <slot></slot>
        </mds-text>
        <mds-icon name="action-close" class="flex-shrink-0 ml-1 bg-adjust-tone rounded-full text-adjust-tone-04"/>
      </Host>
    )
  }
}
