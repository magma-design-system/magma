...
- **Accessibility**: The host exposes a localized `aria-label` describing the previewed URL, and the inner iframe carries its own localized `aria-label`; both interpolate the resolved domain.
- **Close action**: The header dismiss button emits the `mdsUrlViewClose` event and, when the component is nested inside a `mds-modal`, automatically calls `close()` on that ancestor modal.
- **Keyboard activation**: The close control is registered with the shared keyboard manager so it can be triggered by keyboard the same way as a pointer click; the binding is cleaned up on disconnect.
- **Localization**: Header tooltip and ARIA strings are resolved through the shared locale system (el / en / es / it) and can be refreshed at runtime via the `updateLang()` method.
