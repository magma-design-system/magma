...
- **Content zone is centered text**: The `content` slot is centered with `text-align: center` and constrained by a max-width, so headings and descriptions read as a focused message block.
...
- **Themed surface**: The host renders a neutral rounded surface that transitions its background and shadow, picking up theme, contrast, and reduced-motion preferences from the shared preference stylesheets.
