...
const groupedShadows = boxShadowEntries.reduce(
  (acc, [name, token]) => {
...
const Template = () => (
  <div class="grid gap-1200">
    {groupOrder.map(
      (group) =>
        groupedShadows[group] &&
        groupedShadows[group].length > 0 && (
          <div key={group}>
    ...
    <StackedDocuments
      value={getShadowValue(['box-lg', 'box-md', 'lg', 'md', 'DEFAULT', 'default'])}
    />
  </div>
);
