...
if (process.argv.length === 3) {
  ...
  nonInteractive = true;
  console.info(
    `${chalk.yellow('Non-interactive mode')} using ${componentNameArgument} component name`,
  );
}
...
const main = async (): Promise<void> => {
  console.info(
    `This script will ${chalk.green('isolate')} a stencil component and will create package.json if missing into a an isolated stencil project, ready to be published`,
  );
