...
const overwriteDoc = async (componentName: string) => {
  ...
  const readme = await readFile(componentDocPath).catch((error) => {
    throw new Error(chalk.red(error));
  });
  const readmeTemplate = await readFile(join(TEMPLATES_DIR, 'readme.md.hbs')).catch((error) => {
    throw new Error(chalk.red(error));
  });
  ...
  await writeFile(componentDocPath, newDoc, { encoding: 'utf8' })
    .then(() => console.info('File overwrite'))
    .catch((error) => {
      throw new Error(chalk.red(error));
    });
};
