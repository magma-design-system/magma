...
import { readFile, copyFile, writeFile, stat, mkdir, cp, rename } from 'fs/promises'
...
async function askComponentName () {
  ...
  // TODO controllo esistenza componente in src/components
...
const compilePackage = async (componentName) => {
  const exists = !!(await stat(new URL(`${COMPONENTS_PATH}/${componentName}/package.json`, import.meta.url)).catch(() => null))
  if (exists) {
...
const createTempProjectInstance = async (componentName) => {
  ...
  await copyFile(`${PROJECT_PATH}/tailwind.config.js`, `${ISOLATED_PATH}/tailwind.config.js`)
  await copyFile(`${PROJECT_PATH}/tsconfig.json`, `${ISOLATED_PATH}/tsconfig.json`)
  await compileStencil(componentName)
  await mkdir(`${ISOLATED_PATH}/src`, { recursive: true })
  await cp(`${PROJECT_PATH}/src/dictionary`, `${ISOLATED_PATH}/src/dictionary`, { recursive: true })
  await cp(`${PROJECT_PATH}/src/fixtures`, `${ISOLATED_PATH}/src/fixtures`, { recursive: true })
  await cp(`${PROJECT_PATH}/src/interface`, `${ISOLATED_PATH}/src/interface`, { recursive: true })
  await cp(`${PROJECT_PATH}/src/types`, `${ISOLATED_PATH}/src/types`, { recursive: true })
  await cp(`${PROJECT_PATH}/src/components/${componentName}`, `${ISOLATED_PATH}/src/components/${componentName}`, { recursive: true })
  await rename(`${ISOLATED_PATH}/src/components/${componentName}/package.json`, `${ISOLATED_PATH}/package.json`)
  await rename(`${ISOLATED_PATH}/src/components/${componentName}/readme.md`, `${ISOLATED_PATH}/readme.md`)
}
