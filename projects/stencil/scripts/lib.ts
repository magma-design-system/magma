...
const compilePackage = async (componentName: string): Promise<void> => {
  const exists = !!(await stat(join(COMPONENTS_DIR, componentName, 'package.json')).catch(() => {
    return false;
  }));
...
const createPackage = async (componentName: string): Promise<void> => {
  const packageFile = await readFile(join(PROJECT_DIR, 'package.json')).catch((error) => {
    throw Error(chalk.red(error));
  });
  const packageData = JSON.parse(packageFile.toString());
  const packageTemplate = await readFile(join(TEMPLATES_DIR, 'package.json.hbs')).catch((error) => {
    throw Error(chalk.red(error));
  });
  ...
  await writeFile(join(COMPONENTS_DIR, componentName, 'package.json'), packageModule, {
    encoding: 'utf8',
  })
    .then(() => {
...
const compileTemplateFile = async (
  ...
  const exists = !!(await stat(join(COMPONENTS_DIR, componentName, fileName)).catch(() => {
    return false;
  }));
...
const createTemplateFile = async (
  ...
  const packageTemplate = await readFile(join(TEMPLATES_DIR, templateFileName)).catch((error) => {
    throw Error(chalk.red(error));
  });
  ...
  await writeFile(join(COMPONENTS_DIR, componentName, fileName), templateCompiled, {
    encoding: 'utf8',
  })
    .then(() => {
...
const scaffoldStencil = async (componentName: string): Promise<void> => {
  ...
  const stencilTemplate = await readFile(join(TEMPLATES_DIR, fileNameTemplate)).catch((error) => {
    throw Error(chalk.red(error));
  });
  ...
  await writeFile(join(TEMP_PROJECT_DIR, componentName, fileName), stencilConfig, {
    encoding: 'utf8',
  })
    .then(() => {
...
const createTempProjectInstance = async (componentName: string): Promise<void> => {
  ...
  await mkdir(ISOLATED_PATH, { recursive: true }).catch((error) => {
    throw Error(chalk.red(error));
  });

  await copyFile(
    join(PROJECT_DIR, 'tailwind.config.js'),
    join(ISOLATED_PATH, 'tailwind.config.js'),
  ).catch((error) => {
    throw Error(chalk.red(error));
  });
  await copyFile(join(PROJECT_DIR, 'tsconfig.json'), join(ISOLATED_PATH, 'tsconfig.json')).catch(
    (error) => {
      throw Error(chalk.red(error));
    },
  );
  ...
  await mkdir(join(ISOLATED_PATH, 'src'), { recursive: true }).catch((error) => {
    throw Error(chalk.red(error));
  });

  await cp(join(PROJECT_DIR, 'src/dictionary'), join(ISOLATED_PATH, 'src/dictionary'), {
    recursive: true,
  }).catch((error) => {
    throw Error(chalk.red(error));
  });
  await cp(join(PROJECT_DIR, 'src/fixtures'), join(ISOLATED_PATH, 'src/fixtures'), {
    recursive: true,
  }).catch((error) => {
    throw Error(chalk.red(error));
  });
  await cp(join(PROJECT_DIR, 'src/meta'), join(ISOLATED_PATH, 'src/meta'), {
    recursive: true,
  }).catch((error) => {
    throw Error(chalk.red(error));
  });
  await cp(join(PROJECT_DIR, 'src/tailwind'), join(ISOLATED_PATH, 'src/tailwind'), {
    recursive: true,
  }).catch((error) => {
    throw Error(chalk.red(error));
  });
  await cp(join(PROJECT_DIR, 'src/type'), join(ISOLATED_PATH, 'src/type'), {
    recursive: true,
  }).catch((error) => {
    throw Error(chalk.red(error));
  });
  await cp(join(PROJECT_DIR, 'src/common'), join(ISOLATED_PATH, 'src/common'), {
    recursive: true,
  }).catch((error) => {
    throw Error(chalk.red(error));
  });
  await cp(
    join(PROJECT_DIR, 'src/components', componentName),
    join(ISOLATED_PATH, 'src/components', componentName),
    { recursive: true },
  ).catch((error) => {
    throw Error(chalk.red(error));
  });
  ...
  await rename(
    join(ISOLATED_PATH, 'src/components', componentName, 'documentation.json'),
    join(ISOLATED_PATH, 'documentation.json'),
  ).catch((error) => {
    throw Error(chalk.red(error));
  });
  await rename(
    join(ISOLATED_PATH, 'src/components', componentName, 'package.json'),
    join(ISOLATED_PATH, 'package.json'),
  ).catch((error) => {
    throw Error(chalk.red(error));
  });
  await rename(
    join(ISOLATED_PATH, 'src/components', componentName, 'readme.md'),
    join(ISOLATED_PATH, 'readme.md'),
  ).catch((error) => {
    throw Error(chalk.red(error));
  });
};
