...
import {
  loadTypescriptCodeFromMemory,
  makeMatcher,
  transformNodeWith,
  walkAll,
  walkTo,
} from '../helpers/tsFiles';
...
function addStylePropertyGetter(sourceFile: SourceFile, css: string) {
  ...
  const returnStatementRewriter = (node: Node) => {
    ...
    if (
      ...
      classDeclaration.initializer.kind === SyntaxKind.ClassExpression
    ) {
...
function transformSourceToIncludeNewTailwindStyles(
