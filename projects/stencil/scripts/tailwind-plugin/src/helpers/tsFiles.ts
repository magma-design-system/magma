...
export function makeMatcher(
  kind: SyntaxKind,
  {
    ...
    modifier,
    initializer,
  }: { name?: string; modifier?: ModifierSyntaxKind; initializer?: SyntaxKind } = {},
) {
  ...
  return function (context: TransformationContext): Transformer<Node> {
    function visit(node: Node): Node | Array<Node> {
      ...
      if (doesMatch(node, treeLevel)) {
        ...
        debug(
          '[Typescript]',
          'Moving to next tree level',
          treeLevel,
          'looking for',
          SyntaxKind[walkPath[treeLevel].nodeKind],
        );
...
export function transformNodeWith(
  sourceFile: SourceFile,
  walkPath: ASTMatcher[],
  userAction: ASTMatchCallback,
) {
