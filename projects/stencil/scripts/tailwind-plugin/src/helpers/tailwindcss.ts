...
function applyRawEscaping(css: string) {
  return css.replace(/\n/g, '').replace(/`/g, '\\`').replace(/\t/g, ' ').replace(/'/g, '%27');
}

function buildTailwindConfigurationForPostCss(
  configuration: PluginConfigurationOptions,
  filePath: string,
) {
  let optimizeOptions = {};
...
async function getPostcssPluginsWithTailwind(
  configuration: PluginConfigurationOptions,
  filePath: string,
) {
...
async function processTailwindPostcss(
  configuration: PluginConfigurationOptions,
  filename: string,
  sourceCss?: string,
) {
...
export function processSourceTextForTailwindInlineClasses(
...
export async function reduceDuplicatedClassesFromFunctionalComponentInjection(
  opts: PluginConfigurationOptions,
  filename: string,
  componentCss: string,
  injectCss: string,
