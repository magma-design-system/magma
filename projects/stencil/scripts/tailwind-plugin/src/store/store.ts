...
function makeImportAndNameMatcher(filename: string) {
  ...
  return (file: ImportFileReference) =>
    file.imports.indexOf(importMatch) !== -1 || file.name === filename;
}
...
export function registerCssForInjection(filenameRef: string, css: string) {
  ...
  filesWithImportMaps.forEach((file) => {
    if (shouldStoreCss(file)) {
      ...
      debug(
        '[STORE]',
        'Found cross reference for imported file',
        file.name,
        'to store css against',
      );
...
export function getAllExternalCssDependencies(filenameRef: string): CssDependencies {
  ...
  const cssInjection = getImportMatchesForCssFile(filename).reduce<CssDependencies>(
    (deps: CssDependencies, file: ImportFileReference) => {
