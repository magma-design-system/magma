...
function updateComponentFouc(componentDirName: string) {
  ...
  const component =
    documentation.components.find((c) => c.tag === componentDirName) ?? documentation.components[0];
  const deps = component.dependencies ?? [];

  const tagNames = deps.filter((dep) => dep.startsWith('mds-')).sort((a, b) => a.localeCompare(b));
