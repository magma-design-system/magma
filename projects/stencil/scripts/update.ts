 ...
 */
async function getComponents(includeNotPublish: boolean = false): Promise<string[]> {
  let res: string[] = [];
...
async function buildMap(): Promise<void[]> {
  ...
  promises.push(
    ...Array.from(componentsMap.keys()).map((name) => {
...
function getCurrentComponentDependencies(c: string): Promise<Dep> {
  ...
  return d
    ? Promise.resolve(d)
    : readJSON(componentPackagePath(c)).then((json) => json.dependencies);
}
...
function getCurrentComponentVersion(c: string): Promise<string> {
  ...
  return v
    ? Promise.resolve(v)
    : readJSON(componentPackagePath(c))
        .then((p) => p.version)
        .catch(() => {
          throw new Error('not mds component');
        });
}
  ...
  return fetch(endpoint)
    .then((res) => {
      ...
      return Promise.reject(
        new Error(`cant fetch npm package info ${c}\n${res.status}: ${res.statusText}`),
      );
    })
    .then((json: any) => {
      if (!json) return null;
...
function getNpmComponentVersion(c: string): Promise<string> {
  ...
  return fetch(endpoint)
    .then((res) => {
      ...
      return Promise.reject(
        new Error(`cant fetch npm version package ${c}\n${res.status}: ${res.statusText}`),
      );
    })
...
function getNpmComponentDependencies(c: string): Promise<Dep> {
  ...
  return fetch(endpoint)
    .then((res) => {
      ...
      return Promise.reject(
        new Error(`cant fetch npm dependencies of package ${c}\n${res.status}: ${res.statusText}`),
      );
    })
 ...
 */
async function needUpdateComponent(component: string, version: string): Promise<boolean> {
  ...
  if (version === 'minor' && npmVersion.split('.')[1] === currentVersion.split('.')[1]) return true;
  if (version === 'major' && npmVersion.split('.')[0] === currentVersion.split('.')[0]) return true;
  return false;
}
...
async function updateComponentDependencies(name: string): Promise<void> {
  ...
  if (relatedComponent) {
    await Promise.all(
      relatedComponent.map(async (rc) => {
        if (rc === name) {
          throw new Error(`Component ${name} has self dependencies`);
        }
        const pInfo = componentsUpdatedMap.get(rc);
        const versionUpgradeType = getVersionTypeDiff(
          pInfo?.dependencies[`@maggioli-design-system/${name}`],
          v,
        );
  ...
  if (pInfo) {
    Object.entries(pInfo.dependencies).map(async ([dep, vers]) => {
      const currentDepVersion = await getCurrentComponentVersion(dep.split('/')[1] ?? '').catch(
        () => {
          return null;
        },
      );
...
async function syncDep(): Promise<void[]> {
  return Promise.all(
    Array.from(componentsMap.keys()).map((name) => updateComponentDependencies(name)),
  );
}

async function updateCommonDependencies(): Promise<void[]> {
  const commonDeps = (await readJSON(resolve(PROJECT_DIR, 'package.json')).then(
    (p) => p.dependencies,
  )) as Dep;

  return Promise.all(
    Array.from(componentsMap.keys()).map((componentName) => {
      getCurrentComponentDependencies(componentName).then((deps) => {
        // filter dependencies that need to be updated
        const sharedDependencies = Array.from(Object.keys(commonDeps)).filter(
          (cd) => deps[cd] && commonDeps[cd] !== deps[cd],
        );
        ...
        if (sharedDependencies.length) {
          ...
          componentsUpdatedMap.set(componentName, {
            version: componentsUpdatedMap.get(componentName)!.version,
            dependencies: deps,
          });
...
async function main(argv: string[]): Promise<void> {
  ...
  await Promise.all(componentsToBeUpdated.map((c) => updateComponent(c, version)));
  await Promise.all(componentsToBeUpdated.map(updateComponentDependencies));
