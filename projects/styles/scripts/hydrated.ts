...
const main = async () => {
  ...
  const hydratedTemplate = await readFile(join(TEMPLATES_DIR, 'hydrated.css.hbs')).catch(
    (error) => {
  ...
  await writeFile(hydratedFilePath, compiledHydratedComponents, { encoding: 'utf8' })
