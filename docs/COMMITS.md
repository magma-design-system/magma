...
| `doc`    | Documentation only (markdown, JSDoc, READMEs). **Not** `docs`                              |
...
| `refact` | Refactoring with no behavioural or API change. **Not** `refactor`                          |
...
doc(stencil): improve formatting and clarify component usage documentation
...
🚫 refactor(stencil): tidy imports  ← "refactor" is not allowed; use "refact"
🚫 docs(stencil): fix typo          ← "docs" is not allowed; use "doc"
...
2. Pick the **type** using the decision tree above. Remember the custom spellings: `refact` (not `refactor`), `doc` (not `docs`).
